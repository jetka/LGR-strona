    ...
    return (
        ...
        >
            ...
            <div className="relative w-full h-[45vh] md:h-[55vh] overflow-hidden mb-8 md:mb-12">
                <div className="absolute inset-0 scale-105 transition-transform duration-700">
                  <Image 
                    src={getMediaUrl(heroImg)} 
                    alt={post.title} 
                    fill 
                    unoptimized 
                    priority={index === 0} 
                ...
                {post.imageUrls?.length > 1 && (
                    <div className="mb-14 grid grid-cols-1 sm:grid-cols-2 gap-4">
                        {post.imageUrls.slice(1).map((src: string, i: number) => (
                            <div key={i} className="relative w-full h-[300px] md:h-[420px] overflow-hidden rounded-xl">
                                <Image
                                    src={getMediaUrl(src)}
                                    alt={`Zdjęcie ${i + 2}`}
                                    fill
