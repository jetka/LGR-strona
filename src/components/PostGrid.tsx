...
import { Calendar, ChevronDown, ArrowUpDown, SlidersHorizontal, Clock } from "lucide-react";
...
function PostCard({ post, index, basePath }: { post: any; index: number; basePath: string }) {
    const hasGpx = post.category === "INNE" && post.routeData && post.routeData.length > 0;
    const img = !hasGpx ? (post.imageUrls?.[0] || FALLBACK_IMAGES[index % FALLBACK_IMAGES.length]) : null;
...
export default function PostGrid({ posts, sectionTitle, sectionSubtitle, basePath, emptyMessage }: Props) {
    ...
    return (
        <div className="w-full min-h-screen text-gray-200 pt-24 pb-32">
            ...
            {/* ── HEADER ── */}
            <div className="max-w-[1300px] mx-auto px-4 md:px-8 mb-10">
                    ...
                    <div className="flex items-center gap-3 flex-wrap">
                        ...
                        <div className="relative">
                            <select
                                ...
                                className="appearance-none bg-black/50 border border-white/10 rounded-lg px-5 py-3 pr-9 text-sm font-bold uppercase tracking-wider text-white hover:bg-white/5 transition-colors focus:outline-none focus:border-[var(--color-lgr-red)] cursor-pointer"
                            >
