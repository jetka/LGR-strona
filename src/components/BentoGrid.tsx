      ...
      >
          ...
          >
            {latestStart?.imageUrls?.[0] && (
              <div className="absolute inset-0 opacity-30 group-hover:opacity-40 transition-opacity duration-700">
                  <Image src={getMediaUrl(latestStart.imageUrls[0])} alt="Ostatni Start" fill priority unoptimized className="object-cover" sizes="(max-width: 768px) 100vw, 50vw" />
              </div>
            )}
        ...
        >
          <div className="absolute inset-0 opacity-30 group-hover:scale-105 transition-transform duration-700">
              <Image src="/trasyBG.jpg" alt="Trasy LGR" fill priority className="object-cover" sizes="(max-width: 768px) 100vw, 50vw" />
          </div>
            ...
            <div className="absolute inset-0 bg-neutral-900 group-hover:scale-105 transition-transform duration-700 overflow-hidden">
              {latestMedia?.imageUrls?.[0] ? (
                <div className="absolute inset-0">
                    <Image src={getMediaUrl(latestMedia.imageUrls[0])} alt="Najnowsze Media" fill unoptimized className="object-cover" sizes="(max-width: 768px) 100vw, 25vw" />
                </div>
        ...
        <Link href={latestEvent ? `/wydarzenia/${latestEvent.slug}` : "#"} className="col-span-1 md:col-span-3 row-span-1 block h-full">
          ...
          >
            {latestEvent?.imageUrls?.[0] && (
              <div className="absolute inset-0 opacity-30 group-hover:opacity-40 transition-opacity duration-700">
                  <Image src={getMediaUrl(latestEvent.imageUrls[0])} alt="Ostatnie Wydarzenie" fill unoptimized className="object-cover" sizes="100vw" />
              </div>
            )}
